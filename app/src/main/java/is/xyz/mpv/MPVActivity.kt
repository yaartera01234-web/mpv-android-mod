...
class MPVActivity : AppCompatActivity(), MPVLib.EventObserver, TouchGesturesObserver {
    ...
    private fun initListeners() {
        with (binding) {
            ...
            topBackBtn.setOnClickListener { onBackPressedDispatcher.onBackPressed() }
            aspectButton.setOnClickListener { showAspectRatioDialog(null) }
            aspectButton.setOnLongClickListener { openAdvancedMenu(pauseForDialog()); true }
            screenshotBtn.setOnClickListener { takeScreenshot() }
            brightnessBtn.setOnClickListener { showSliderPanel(SliderMode.Brightness) }
            volumeBtn.setOnClickListener { showSliderPanel(SliderMode.Volume) }
            barLockBtn.setOnClickListener { lockUI() }
            barMenuBtn.setOnClickListener { openTopMenu() }
            delayButton.setOnClickListener { showAudioDelayDialog(null) }
            delayButton.setOnLongClickListener { openAdvancedMenu(pauseForDialog()); true }
            sliderSeekbar.setOnSeekBarChangeListener(sliderSeekbarListener)
    ...
    fun hideControls() {
        ...
        // the slider panel belongs to the control bar
        sliderHideRunnable?.let { fadeHandler.removeCallbacks(it) }
        sliderHideRunnable = null
        sliderMode = null
        binding.sliderPanel.animate().cancel()
    ...
    override fun onConfigurationChanged(newConfig: Configuration) {
        ...
        binding.controls.updateLayoutParams<MarginLayoutParams> {
            val barInset = Utils.convertDp(this@MPVActivity, 10f)
    ...
    private fun openAdvancedMenu(restoreState: StateRestoreCallback) {
        ...
        val buttons: MutableList<MenuItem> = mutableListOf(
                ...
                MenuItem(R.id.aspectBtn) {
                    showAspectRatioDialog(restoreState); false
                },
        )
        ...
        buttons.add(MenuItem(R.id.audioDelayBtn) {
            showAudioDelayDialog(restoreState); false
        })
    ...
    private fun updateAudioUI() {
        ...
        // buttons that only make sense for video / for files with audio
        binding.aspectButton.visibility = if (shouldUseAudioUI) View.GONE else View.VISIBLE
        binding.screenshotBtn.visibility = if (shouldUseAudioUI) View.GONE else View.VISIBLE
        binding.delayButton.visibility = if (player.aid == -1) View.GONE else View.VISIBLE
        ...
        if (useAudioUI) {
            ...
            // (INVISIBLE, not GONE, so the icons in the top bar stay right-aligned)
            if (showMediaTitle) {
                binding.controlsTitleGroup.visibility = View.VISIBLE
                Utils.viewGroupReorder(binding.controlsTitleGroup, arrayOf(R.id.fullTitleTextView))
                updateMetadataDisplay()
            } else {
                binding.controlsTitleGroup.visibility = View.INVISIBLE
            }
    ...
    /* ===================== control bar: slider panel ===================== */

    private enum class SliderMode { Brightness, Volume }

    private var sliderMode: SliderMode? = null
    private var sliderMaxValue = 100
    private var sliderHideRunnable: Runnable? = null

    private val sliderSeekbarListener = object : SeekBar.OnSeekBarChangeListener {
        override fun onProgressChanged(seekBar: SeekBar, progress: Int, fromUser: Boolean) {
            when (sliderMode) {
                SliderMode.Brightness -> {
                    lastScreenBrightness = progress
                    updateScreenBrightness()
                    binding.sliderValue.text = getString(R.string.ui_brightness, progress)
                }
                SliderMode.Volume -> {
                    audioManager?.setStreamVolume(STREAM_TYPE, progress, 0)
                    val percent = if (sliderMaxValue > 0) 100 * progress / sliderMaxValue else 0
                    binding.sliderValue.text = getString(R.string.ui_volume, percent)
                }
                null -> return
            }
            scheduleSliderHide()
        ...
        override fun onStartTrackingTouch(seekBar: SeekBar) {}
        override fun onStopTrackingTouch(seekBar: SeekBar) = scheduleSliderHide()
    }

    /** Shows the on-screen brightness/volume slider above the control bar. */
    private fun showSliderPanel(mode: SliderMode) {
        when (mode) {
            SliderMode.Brightness -> {
                val current = ((Utils.getScreenBrightness(this) ?: 0.5f) * 100).roundToInt().coerceIn(0, 100)
                sliderMaxValue = 100
                binding.sliderIcon.setImageResource(R.drawable.ic_brightness_6_24dp)
                binding.sliderSeekbar.max = 100
                binding.sliderSeekbar.progress = current
                binding.sliderValue.text = getString(R.string.ui_brightness, current)
            }
            SliderMode.Volume -> {
                val am = audioManager
                val max = if (am == null || am.isVolumeFixed) 0 else am.getStreamMaxVolume(STREAM_TYPE)
                if (max <= 0) {
                    showToast(getString(R.string.ui_volume_unavailable))
                    return
                }
                val current = am!!.getStreamVolume(STREAM_TYPE)
                sliderMaxValue = max
                binding.sliderIcon.setImageResource(R.drawable.ic_volume_up_24dp)
                binding.sliderSeekbar.max = max
                binding.sliderSeekbar.progress = current
                binding.sliderValue.text = getString(R.string.ui_volume, 100 * current / max)
            }
        }

        sliderMode = mode
        // keep the control bar on screen while the user is sliding
        showControls()
        binding.sliderPanel.animate().cancel()
        binding.sliderPanel.alpha = 1f
        binding.sliderPanel.visibility = View.VISIBLE
        scheduleSliderHide()
    }

    private fun scheduleSliderHide() {
        sliderHideRunnable?.let { fadeHandler.removeCallbacks(it) }
        val runnable = Runnable { hideSliderPanel() }
        sliderHideRunnable = runnable
        fadeHandler.postDelayed(runnable, SLIDER_DISPLAY_TIMEOUT)
    }

    private fun hideSliderPanel() {
        sliderHideRunnable?.let { fadeHandler.removeCallbacks(it) }
        sliderHideRunnable = null
        sliderMode = null
        if (binding.sliderPanel.visibility != View.VISIBLE)
            return
        binding.sliderPanel.animate().cancel()
        binding.sliderPanel.animate().alpha(0f).setDuration(CONTROLS_FADE_DURATION).withEndAction {
            binding.sliderPanel.visibility = View.GONE
            // let the normal controls timeout take over again
            showControls()
        }.start()
    }

    /* ===================== control bar: audio delay ===================== */

    /** Audio delay picker (-600 .. +600), used by the control bar and the advanced menu. */
    private fun showAudioDelayDialog(restoreState: StateRestoreCallback?) {
        val picker = DecimalPickerDialog(-600.0, 600.0)
        genericPickerDialog(picker, R.string.audio_delay, "audio-delay") { restoreState?.invoke() }
    }

    /* ===================== control bar: aspect ratio ===================== */

    /** Aspect ratio picker, shared by the control bar button and the advanced menu. */
    private fun showAspectRatioDialog(restoreState: StateRestoreCallback?) {
        ...
        with (AlertDialog.Builder(this)) {
            ...
            setItems(R.array.aspect_ratio_names) { dialog, item ->
                ...
                dialog.dismiss()
                showToast(getString(R.string.ui_aspect_set, names[item]))
            }
            setOnDismissListener { restoreState?.invoke() }
            create().show()
        }
    }
    ...
    private fun takeScreenshot() {
        if (player.vid == -1) {
            showToast(getString(R.string.ui_screenshot_no_video))
            return
        }
        val dir = File(cacheDir, "screenshots").apply { mkdirs() }
        val stamp = SimpleDateFormat("yyyyMMdd_HHmmss", Locale.US).format(Date())
        val tmp = File(dir, "mpv_$stamp.png")
        MPVLib.setPropertyString("screenshot-format", "png")
        MPVLib.command(arrayOf("screenshot-to-file", tmp.absolutePath, "video"))
        publishScreenshot(tmp, 0)
    }

    /** mpv writes the screenshot from its own thread, so wait for the file to show up. */
    private fun publishScreenshot(file: File, attempt: Int) {
        if (file.exists() && file.length() > 0L) {
            val location = saveScreenshotTo(file)
            if (location != null)
                showToast(getString(R.string.ui_screenshot_saved, location))
            else
                showToast(getString(R.string.ui_screenshot_failed))
            return
        }
        if (attempt < 25)
            fadeHandler.postDelayed({ publishScreenshot(file, attempt + 1) }, 200)
        else
            showToast(getString(R.string.ui_screenshot_failed))
    }

    private fun saveScreenshotTo(file: File): String? {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q)
            return saveScreenshotToMediaStore(file)
        // pre-Q: try the public Pictures folder, fall back to the app's own folder
        try {
            ...
            Log.w(TAG, "screenshot: could not write to Pictures: $e")
        }
        return moveScreenshotToAppDir(file)
    }

    @RequiresApi(Build.VERSION_CODES.Q)
    private fun saveScreenshotToMediaStore(file: File): String? {
        val relativePath = Environment.DIRECTORY_PICTURES + "/mpv"
        ...
        if (uri == null) {
            Log.w(TAG, "screenshot: MediaStore insert failed")
            return moveScreenshotToAppDir(file)
        }
        try {
            ...
            Log.w(TAG, "screenshot: MediaStore write failed: $e")
    ...
    private fun moveScreenshotToAppDir(file: File): String? {
        return try {
            val parent = getExternalFilesDir(Environment.DIRECTORY_PICTURES) ?: cacheDir
            parent.mkdirs()
            val dest = File(parent, file.name)
            file.copyTo(dest, overwrite = true)
            file.delete()
            dest.absolutePath
        } catch (e: Exception) {
            Log.w(TAG, "screenshot: could not move file: $e")
            null
        }
    }
    ...
    private fun eventPropertyUi(property: String, value: Boolean) {
        ...
        when (property) {
            "pause" -> {
                ...
                // pause karte hi control bar aa jaye (MX style)
    ...
    companion object {
        ...
        private const val SLIDER_DISPLAY_TIMEOUT = 3000L
