...
internal class TouchGestures(private val observer: TouchGesturesObserver) {
    ...
    companion object {
        ...
        // how long a single tap waits before it acts (must be > TAP_DURATION)
        private const val TAP_SETTLE = TAP_DURATION + 60
    ...
    /** which tap zone a horizontal position belongs to: -1 = left, 0 = center, 1 = right */
    ...
    /** fixed single tap behaviour: left = seek back 10s, center = play/pause, right = seek fwd 10s */
    private fun scheduleSingleTap(zone: Int) {
        cancelPendingTap()
        val r = Runnable {
            pendingTap = null
            when (zone) {
                -1 -> sendPropertyChange(PropertyChange.SeekFixed, -1f)
                0 -> sendPropertyChange(PropertyChange.PlayPause, 0f)
                else -> sendPropertyChange(PropertyChange.SeekFixed, 1f)
            }
        }
        pendingTap = r
        handler.postDelayed(r, TAP_SETTLE)
    }
    ...
    private fun processTap(p: PointF): Boolean {
        ...
        if (now - lastDownTime >= TAP_DURATION) {
            // finger was held too long -> not a tap. let the activity handle it (controls toggle)
            lastTapTime = 0
            cancelPendingTap()
            return false
        }
        if (now - lastTapTime < TAP_DURATION) {
            ...
            lastTapTime = 0
            cancelPendingTap()
            return when (zoneOf(p.x)) {
                -1 -> tapGestureLeft?.let { sendPropertyChange(it, -1f); true } ?: false
                0 -> tapGestureCenter?.let { sendPropertyChange(it, 0f); true } ?: false
                else -> tapGestureRight?.let { sendPropertyChange(it, 1f); true } ?: false
            }
        } else {
            lastTapTime = now
            if (singleTapZones) {
                scheduleSingleTap(zoneOf(p.x))
                return true
            }
        }
        return false
    ...
    private fun processMovement(p: PointF): Boolean {
        ...
        when (state) {
            ...
            State.Down -> {
                ...
                if (state != State.Down) {
                    cancelPendingTap() // a drag is not a tap
    ...
    fun syncSettings(prefs: SharedPreferences, resources: Resources) {
        ...
        singleTapZones = prefs.getBoolean("gesture_single_tap_zones",
            resources.getBoolean(R.bool.pref_gesture_single_tap_zones_default))
    }
    ...
    fun onTouchEvent(e: MotionEvent): Boolean {
        ...
        when (e.action) {
            ...
            MotionEvent.ACTION_DOWN -> {
                // naya touch shuru: pichla pending single tap cancel
                // (warna tap ke foran baad drag/hold karne pe ghalti se seek ho jata hai)
                cancelPendingTap()
